...
call :compile globalerp "Компиляция GlobalERP (интеграция с GLOBAL)"
call :compile goz "Компиляция GOZ (отчеты для госзаказов)"
call :compile mnf "Компиляция mnf (доработки контура производственного планирования)"
call :compile paymentsdistribution "Компиляция PaymentsDistribution (бюджет заказа для ПЭО)"
...
call :compile report_dsql "Компиляция report_dsql (DSQL-Отчеты)"
call :compile rkm "Компиляция RKM (доработки по РКМ)"
call :compile templan "Компиляция TemPlan (ТемПлан)"
call :compile zar "Компиляция Zar (доработки по зарплате)"
