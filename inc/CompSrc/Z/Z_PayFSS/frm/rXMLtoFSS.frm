...
// --- Больничный лист
  ...
  BlProd           //  Признак продолжающегося больничного
...
!
  ...
  LiveCode         //  Код условия проживания
!------------------------------------------------------------------------------
  RefNpayOtherNo   //  справка от другого родителя о неполучении пособия номер  для версии 1.7.1
  RefNpayOtherDt   //  справка от другого родителя о неполучении пособия дата
...
!------------------------------------------------------------------------------
  ...
  WardRefNo        //  усыновление/опека
  WardRefDt        //  усыновление/опека
  RefFamilyNo      //  договор о передаче в приемную семью номер  для версии 1.7.1
  RefFamilyDt      //  договор о передаче в приемную семью дата
...
//------- условия исчисления
 ...
 // --- Больничный лист
  ...
  Reason2          //  Доп код причины нетрудоспособности
  Reason3          //  Код изменения причины нетрудоспособности
  LnCode           //  № больничного на основном месте работы    для разных версий
...
.}
.{?INTERNAL; ( Version = '1.7.2' ) and ( VidReestr = cn_vpBornChild )
    <REF_FAMILY_NO>^</REF_FAMILY_NO>
    <REF_FAMILY_DT>^</REF_FAMILY_DT>
.}
.} // ( RefFamilyNo <> '' )
...
.}
!
.{?INTERNAL; ( Version = '1.7.2' ) and ( oCertNo <> '' )
! справка о рождении
    <O_CERT_NO>^</O_CERT_NO>
    <O_CERT_DT>^</O_CERT_DT>
.}
...
.}
.{?INTERNAL; ( Version = '1.7.1' ) and  ( VidReestr = cn_vpOtpusk )
    <CALC_DAYS>^</CALC_DAYS>
.}
!
.{?INTERNAL; ( Form1Dt <> '' )
...
.}
!
.{?INTERNAL; ( Version = '1.7.2' ) and ( VidReestr = cn_vpNetrud )
    <FSS_PAYMENT_FLAG>^</FSS_PAYMENT_FLAG>
.}
!
.{?INTERNAL; ( Version = '1.7.1' ) or ( FssPayFlag = 1 )
    <DT1>^</DT1>
    <DT2>^</DT2>
.}
