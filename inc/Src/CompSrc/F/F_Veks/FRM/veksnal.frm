...
.Var
  ...
  Kol_Com : longint;
  Kol_Mol : longint;
  MOL_Post : array [1..2] of string;
  MOL_FIO : array [1..2] of string;
  Com_Post : array [1..2] of string;
  Com_FIO : array [1..2] of string;
  Lico_Post : string;
  Lico_Name : string;
  Itog_num : longint;
  Itog_kol : double;
  ...
  PageItog_sum : double;
.EndVar
.procedure GetSignerInfo(sRole : string; var sName, sPost : string);
begin
  sName := '';
  sPost := '';
  ...
  if (iFeeSigners.RestrictFeeByRole(1, sRole))
    if (iFeeSigners.GetSignerFirst)
    {
      sName := iFeeSigners.GetSignerFIO(1);
      sPost := iFeeSigners.GetSignerPosition(1);
    }

  iFeeSigners.ClearRestriction;
end.
#declare GetSignerArray(sRole,lNum,lKol,aName,aPost)
  for (Num := 1; Num <= Count(#aName); Num++)
  {
    #aName[Num] := '';
    #aPost[Num] := '';
  }
  ...
  if (iFeeSigners.FeeSignersIsValid)
  {
    if (iFeeSigners.RestrictFeeByRole(1, #sRole))
      if (iFeeSigners.GetSignerFirst)
        ...
        {
          Num++;
          #aName[Num] := iFeeSigners.GetSignerFIO(1);
          #aPost[Num] := iFeeSigners.GetSignerPosition(1);
        }
...
.begin
  ...
  iFeeSigners.FindFeeSignersByNRec(Подписанты_NRec);

  // МОЛ
  #GetSignerArray(cgRole_Otvetstv,2,Kol_Mol,MOL_FIO,MOL_Post);
  // Члены комиссии
  #GetSignerArray(cgRole_ChlenCom,3,Kol_Com,Com_FIO,Com_Post);
end.
.Fields
  Собственная_организации_наим
  Собственная_организации_ОКПО
...
!
  if (Itog_num > 0, DoubleToStr(Itog_num, '\m4'), '')
  if (Itog_num > 0, DoubleToStr(Itog_kol, '\m4'), '')
  if (Itog_num > 0, DoubleToStr(Itog_sum, '\2p\m4'), '')
  if (Itog_num > 0, DoubleToStr(Itog_sum, '\2p88'), '')
! председатель комиссии
  Lico_Post
  Lico_Name
! члены комиссии
  if (Num = 1, 'Члены комиссии:', '')
  Com_Post[Num]
  Com_FIO[Num]
!
  if (Itog_num > 0, '1', '')
  if (Itog_num > 0, String(Itog_num), '')
! МОЛ
  MOL_Post[Num]
  MOL_FIO[Num]
! проверяющее лицо
...

     Итого по описи:
                    ...
                    в) на сумму, фактически                Д@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Д
                                                                                                                             (прописью)
                     _____________________________________________________________________________________________________________________________________________________________  руб   Д@@@@@@@@@@@@@@@@@@@@@@@@@@ Д коп

     Все подсчеты итогов по строкам, страницам и в целом по инвентаризационной описи проверены.

.begin
  GetSignerInfo(cgRole_PredCom, Lico_Name, Lico_Post);
end.
...
.}
     ...
     Все ценные бумаги и бланки строгой отчетности, поименованные в настоящей инвентаризационной описи с №  Д@~@@@@@@@@@@@@@@@@@@@@@ Д по №  Д@~@@@@@@@@@@@@@@@@@@@@@ Д комиссией проверены в натуре в моем (нашем) присутствии и
...
.begin
  GetSignerInfo(cgRole_ProvLico, Lico_Name, Lico_Post);
end.
     Указанные в настоящей описи данные и подсчеты проверил      Д@@@@@@@@@@@@@@@@@@@@@@@@ Д        ________________________         Д@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Д
                                                                       (должность)                       (подпись)                                              (расшифровка подписи)
...
.begin
  GetSignerInfo(cgRole_Direct, Lico_Name, Lico_Post);
end.
                                             Д@@@@@@@@@@@@@@@@@@@@@@@@ Д        ________________________         Д@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Д
                                                   (должность)                       (подпись)                                              (расшифровка подписи)
