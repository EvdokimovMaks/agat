...
!!!!!!!!!!!!!!!!!!!!!!! ПУ-6 с 2012 года!!!!!!!!!!!!!!!!!!!!!!!!!!!
.linkform 'formPU6_2012_LMAGNIT' prototype is 'FORMPU6'
.nameinlist 'Форма ПУ-6 от 1.01.2012 (файл для магнитных носителей)'
.var
 pach_nmb   : word
 nomer      : word
 KolNach    : integer
...
if(Trim(ProfCode)<>'' and CharF <> 2, UpCase(Trim(ProfCode)),' ')
if(Trim(VidDeyal)<>'',UpCase(Trim(VidDeyal)),' ')
if(Trim(Cod_VidDeyal)<>'',UpCase(Trim(Cod_VidDeyal)),' ')
if(Trim(Dop_Pokazat)<>'' and CharF <> 2,UpCase(Trim(Dop_Pokazat))+'/',if((Trim(Dop_Rned)<>''),'0/',''))
if((Trim(Dop_Rned)<>'')and CharF <> 2 ,UpCase(Trim(Dop_Rned)),' ')
EndLineStr
.endfields
.begin
...
.nameinlist 'Форма ПУ-6 от 1.04.2012 (файл для магнитных носителей)'
.var
 pach_nmb   : word;
 nomer      : word;
 KolNach    : integer;
