...
################################################################################
##                                                                            ##
## JavaScript, выполняющий выгрузку позиций ПСД из Excel-файла                ##
...
// заданному типу
var gsSheetName = 'SpCES';                   // Наименование Excel-листа, содержащего данные для позиций ПСД
...
var gliClnNmb_KatStroyCode       = 4;        // Номер колонки, соответствующей полю "Код подъобъекта строительства"
var gliClnNmb_SSOAlbumPartCipher = 6;        // Номер колонки, соответствующей полю "Шифр раздела альбома ССО"
var gliClnNmb_PID                = 7;        // Номер колонки, соответствующей полю "PID проекта"
var gliClnNmb_Name               = 8;        // Номер колонки, соответствующей полю "Краткое наименование МТР"
var gliClnNmb_Kol                = 9;        // Номер колонки, соответствующей полю "Количество"
...
var gliXMLFileGAL_Node_Clt_SpCES;            // Handle узла-коллекции "Позиции ПСД"
...
//------------------------------------------------------------------------------
// Добавление позиции ПСД в XML-файл в формате ГАЛАКТИКА-ERP
// Параметры: 
//   abCheckExists   - Признак проверки существования объекта
//   asID            - Идентификатор
//   asССESDoc       - Значение поля "Ссылка на ПСД"
//   asСKatStroy     - Значение поля "Ссылка на подъобъект строительства"
//   asCSSOAlbumPart - Значение поля "Ссылка на раздел альбома ССО"
//   asPID           - Значение поля "PID проекта"
//   asCMC           - Значение поля "Ссылка на МЦ"
//   asName          - Значение поля "Наименование"
//   asKol           - Значение поля "Количество"
// Результат:
//   0 - объект не добавлялся, 1 - объект добавлен

function XMLFileGAL_AddSpCES(abCheckExists, asID, asССESDoc, asСKatStroy, asCSSOAlbumPart,
...
var vsName;                            //
  ...
  // Создание значений свойств
  XMLFileGAL_Add_Tag_PropValue(vliXMLFileGAL_Node_Clt_PropValues, 'cCESDoc'      , asССESDoc      , 'CESDoc');
  XMLFileGAL_Add_Tag_PropValue(vliXMLFileGAL_Node_Clt_PropValues, 'cKatStroy'    , asСKatStroy    , 'KatStroy');
...
var vsKatStroyCode;       // Значение поля "Код подъобъекта строительства"
var vsSSOAlbumPartCipher; // Значение поля "Шифр раздела альбома ССО"
var vsPID;                // Значение поля "PID проекта"
var vsName;               // Значение поля "Краткое наименование МТР"
var vsKol;                // Значение поля "Количество"
//
var vsССESDoc;            // Значение поля "Ссылка на ПСД"
var vsСKatStroy;          // Значение поля "Ссылка на подъобъект строительства"
var vsCSSOAlbumPart;      // Значение поля "Ссылка на раздел альбома ССО"
var vsCMC;                // Значение поля "Ссылка на МЦ"
  ...
  if (goWorkBook.ActiveSheet.Name != gsSheetName)
  {
    Message(
      ...
      'Данные для позиций ПСД извлекаются только из указанной выше вкладки.');
  ...
  do
  {
    ...
    vsССESDoc = GetObjID_CESDoc(vsNoDoc);
    ...
    if (vsKatStroyCode != '')
      vsСKatStroy = GetObjID_KatStroy(vsKatStroyCode);
